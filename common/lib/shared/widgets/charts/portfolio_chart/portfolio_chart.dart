...
class PortfolioChart extends StatelessWidget {
  ...
  const PortfolioChart({Key? key, required this.portfolioKey})
      : super(key: key);

  Widget assetChart(PortfolioChartController controller) {
  ...
  Widget build(BuildContext context) {
    return GetX<PortfolioChartController>(
        tag: portfolioKey.toString(),
        global: false,
        init: PortfolioChartController(portfolioKey: portfolioKey),
        builder: (controller) {
  ...
  Widget fundamentals(PortfolioChartController controller) {
    ...
    return HorizontalInfo(columns: [
      InfoColumn(items: [
        InfoItem(
          ...
          value: assetStat.putCallRatio.toString(),
        ),
  ...
  shimmer() {
    return Column(
      children: const [HistoricalChartShimmer()],
    );
  }
}
