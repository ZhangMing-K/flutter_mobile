...
class TrackballBehavior {
  ...
  ///
  /// Defaults to 'point'.
  void showSecondTrackball(dynamic x, double y,
      [String coordinateUnit = 'point']) {
    final SfCartesianChartState chartState = _chartState!;
    final TrackballBehaviorRenderer trackballBehaviorRenderer2 =
        chartState._trackballBehaviorRenderer2;
    final List<CartesianSeriesRenderer> visibleSeriesRenderer =
        chartState._chartSeries.visibleSeriesRenderers;
    final CartesianSeriesRenderer seriesRenderer = visibleSeriesRenderer[0];
    if ((trackballBehaviorRenderer2._trackballPainter != null ||
            builder != null) &&
        activationMode != ActivationMode.none) {
      final ChartAxisRenderer xAxisRenderer = seriesRenderer._xAxisRenderer!;
      if (coordinateUnit != 'pixel') {
      ...
      if (trackballBehaviorRenderer2._trackballPainter != null) {
        trackballBehaviorRenderer2._isTrackballTemplate = false;
        trackballBehaviorRenderer2._generateAllPoints(Offset(x.toDouble(), y));
      } else if (builder != null && (!trackballBehaviorRenderer2._isMoving)) {
        trackballBehaviorRenderer2
            ._showTemplateTrackball(Offset(x.toDouble(), y));
      }
    }

    if (trackballBehaviorRenderer2._trackballPainter != null) {
      trackballBehaviorRenderer2._trackballPainter!.canResetPath = false;
      chartState._repaintNotifiers['trackball2']!.value++;
    }
  }
  ...
  /// Hides the SECOND trackball if it is displayed.
  void hideSecondTrackball() {
    final SfCartesianChartState chartState = _chartState!;
    final TrackballBehaviorRenderer trackballBehaviorRenderer =
        chartState._trackballBehaviorRenderer2;
    if (trackballBehaviorRenderer._trackballPainter != null &&
      ...
      if (!_chartState!._isTouchUp) {
        trackballBehaviorRenderer._trackballPainter!.chartState
            ._repaintNotifiers['trackball2']!.value++;
        trackballBehaviorRenderer._trackballPainter!.canResetPath = true;
      } else {
        final double duration =
            (hideDelay == 0 && chartState._enableDoubleTap) ? 200 : hideDelay;
...
// ignore: unnecessary_null_comparison
  ...
  void _triggerTrackballRenderCallback() {
    if (_chart.onTrackballPositionChanging != null) {
      ...
      for (index = _chartState._chartPointInfo.length - 1;
        ...
        if (_chartState._tapDownEvents.isNotEmpty &&
            _chartState._tapDownEvents[0].pointer ==
                _chartState.currentTapIndex) {
          ...
          chartPoint.chartPointInfo = _ChartPointInfo();
          _chart.onTrackballPositionChanging!(chartPoint);
        }
      }
    }
    if (_chart.onSecondTrackballPositionChanging != null) {
      _chartState._chartPointInfo2 =
          _chartState._trackballBehaviorRenderer2._chartPointInfo;
      int index;
      for (index = _chartState._chartPointInfo2.length - 1;
          index >= 0;
          index--) {
        TrackballArgs chartPoint;
        chartPoint = TrackballArgs();
        chartPoint.chartPointInfo = _chartState._chartPointInfo2[index];
        if (_chartState._tapDownEvents.length > 1 &&
            _chartState._tapDownEvents[1].pointer ==
                _chartState.currentTapIndex) {
          _chart.onSecondTrackballPositionChanging!(chartPoint);
          _chartState._chartPointInfo2[index].label =
              chartPoint.chartPointInfo.label;
          _chartState._chartPointInfo2[index].header =
              chartPoint.chartPointInfo.header;
          if (_chartState._chartPointInfo2[index].label == null ||
              _chartState._chartPointInfo2[index].label == '') {
            _chartState._chartPointInfo2.removeAt(index);
            _visiblePoints.removeAt(index);
          }
  ...
  void onTouchDown(double xPos, double yPos) {
    final int currentTapIndex =
        _trackballBehavior._chartState?.currentTapIndex ?? 0;
    final events = _trackballBehavior._chartState?._tapDownEvents;
    if (events!.length == 1 && events[0].pointer > 0) {
      _trackballBehavior.show(xPos, yPos, 'pixel');
    } else if (events.length == 2) {
      if (events[0].pointer > 0 &&
          events[1].pointer == -1 &&
          events[0].pointer == currentTapIndex) {
        _trackballBehavior.show(xPos, yPos, 'pixel');
      }
      if (events[1].pointer > 0 &&
          events[0].pointer == -1 &&
          events[1].pointer == currentTapIndex) {
        _trackballBehavior.showSecondTrackball(xPos, yPos, 'pixel');
      }
      if (events[0].pointer > 0 && events[1].pointer > 0) {
        if (events[0].pointer == currentTapIndex) {
          _trackballBehavior.show(xPos, yPos, 'pixel');
        }
        if (events[1].pointer == currentTapIndex) {
          _trackballBehavior.showSecondTrackball(xPos, yPos, 'pixel');
        }
      }
    }
  ...
  void onTouchUp(double xPos, double yPos) {
    ...
    if (events!.length == 1 &&
      ...
      if (events[0].pointer > 0 &&
          ...
          events[0].pointer == currentTapIndex) {
        _trackballBehavior.hide();
      } else if (events[0].pointer == -1 &&
          events[1].pointer > 0 &&
          events[1].pointer == currentTapIndex) {
        _trackballBehavior.hideSecondTrackball();
      } else if (events[0].pointer > 0 && events[1].pointer > 0) {
        if (events[0].pointer == currentTapIndex) {
          _trackballBehavior.hide();
        }
        if (events[1].pointer == currentTapIndex) {
