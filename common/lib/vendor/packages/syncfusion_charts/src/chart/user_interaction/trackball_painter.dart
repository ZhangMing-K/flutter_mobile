...
class _TrackballPainter extends CustomPainter {
  _TrackballPainter(
      ...
      required this.behaviorRenderer})
      : chart = chartState._chart,
        super(repaint: valueNotifier);
  final SfCartesianChartState chartState;
  final SfCartesianChart chart;
  final TrackballBehaviorRenderer behaviorRenderer;
  ...
  @override
  void paint(Canvas canvas, Size size) => behaviorRenderer.onPaint(canvas);
  ...
  void _drawTrackball(Canvas canvas) {
    ...
    if (!_isSeriesAnimating()) {
      ...
      chartPointInfo = trackballBehaviorRenderer._chartPointInfo;
      _markerShapes = trackballBehaviorRenderer._markerShapes;
      _visiblePoints = trackballBehaviorRenderer._visiblePoints;
      isRangeSeries = trackballBehaviorRenderer._isRangeSeries;
      isBoxSeries = trackballBehaviorRenderer._isBoxSeries;
      ...
      for (int index = 0; index < chartPointInfo.length; index++) {
        ...
        final Offset pos = trackballBehaviorRenderer._tapPosition;
      ...
      if (!canResetPath &&
        ...
        trackballBehaviorRenderer._drawLine(
            canvas,
            trackballBehaviorRenderer._linePainter(trackballLinePaint),
            chartPointInfo.indexOf(trackLinePoint));
      }
// ignore: unnecessary_null_comparison
      if (_tooltipTop != null && _tooltipTop.isNotEmpty) {
        _tooltipPosition = trackballBehaviorRenderer._smartTooltipPositions(
            _tooltipTop,
            _tooltipBottom,
            _xAxesInfo,
            _yAxesInfo,
      ...
      for (int index = 0; index < chartPointInfo.length; index++) {
        trackballBehaviorRenderer._trackballMarker(index);

        if (_markerShapes != null &&
            _markerShapes!.isNotEmpty &&
            _markerShapes!.length > index) {
          trackballBehaviorRenderer._renderTrackballMarker(
  ...
  Size _getTooltipSize(double height, double width, int index) {
    ...
    if (isGroupMode) {
      ...
      for (int i = 0; i < chartPointInfo.length; i++) {
        pos = behaviorRenderer._tapPosition;
