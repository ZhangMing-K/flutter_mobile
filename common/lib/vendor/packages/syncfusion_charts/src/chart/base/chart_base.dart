...
class SfCartesianChart extends StatefulWidget {
  ...
  SfCartesianChart(
      ...
      this.onSecondTrackballPositionChanging,
  ...
  final ChartTrackballCallback? onSecondTrackballPositionChanging;
  ...
  ///}
  ///void zoom(ZoomPanArgs args) {
  ///    debugPrint(args.currentZoomPosition);
  ///}
  ///```
  ...
  ///    return Container(
  ///        child: SfCartesianChart(
  ///            onSelectionChanged: (SelectionArgs args) => debugPrint(args.selectedColor),
  ///        ));
  ///}
...
class SfCartesianChartState extends State<SfCartesianChart>
  ...
  late List<PointerEvent> _tapDownEvents;

  /// current tap index
  late int currentTapIndex;
  ...
  //ignore: prefer_final_fields
  List<_ChartPointInfo> _chartPointInfo2 = <_ChartPointInfo>[];

  late ZoomPanBehaviorRenderer _zoomPanBehaviorRenderer;

  late TrackballBehaviorRenderer _trackballBehaviorRenderer;
  late TrackballBehaviorRenderer _trackballBehaviorRenderer2;
  ...
  late TrackballMarkerSettingsRenderer _trackballMarkerSettingsRenderer2;
  ...
  void _initializeDefaultValues() {
    ...
    _repaintNotifiers = <String, ValueNotifier<int>>{
      ...
      'trackball2': ValueNotifier<int>(0),
    ...
    _trackballBehaviorRenderer2 = TrackballBehaviorRenderer(this);
    ...
    _trackballMarkerSettingsRenderer2 = TrackballMarkerSettingsRenderer(
        widget.trackballBehavior.markerSettings);
  ...
  void _redraw() {
    ...
    if (_trackballBehaviorRenderer2._trackballPainter?.timer != null) {
      _trackballBehaviorRenderer2._trackballPainter?.timer!.cancel();
    }
...
class _ContainerArea extends StatelessWidget {
  ...
  void _performPointerDown(PointerDownEvent event) {
    ...
    if (chart.trackballBehavior != null &&
      ...
      if (_chartState._tapDownEvents.length < 2) {
        if (_chartState._tapDownEvents.isEmpty) {
          _chartState._tapDownEvents.add(event); // first finger event
        } else if (_chartState._tapDownEvents.length == 1 &&
            _chartState._tapDownEvents[0].pointer == -1) {
          _chartState._tapDownEvents[0] = event; // first finger event update
        } else if (_chartState._tapDownEvents.length == 1 &&
            _chartState._tapDownEvents[0].pointer > 0) {
          _chartState._tapDownEvents.add(event); // second finger event
        }
      } else if (_chartState._tapDownEvents.length == 2) {
        if (_chartState._tapDownEvents[0].pointer == -1 &&
            _chartState._tapDownEvents[1].pointer > 0) {
          _chartState._tapDownEvents[0] = event;
        } else if (_chartState._tapDownEvents[1].pointer == -1 &&
            _chartState._tapDownEvents[0].pointer > 0) {
          _chartState._tapDownEvents[1] = event;
        } else if (_chartState._tapDownEvents[0].pointer == -1 &&
            _chartState._tapDownEvents[0].pointer == -1) {
          _chartState._tapDownEvents[0] = event;
        }
      }
  ...
  void _performPointerMove(PointerMoveEvent event) {
    _chartState.currentTapIndex = event.pointer;
    _renderingDetails.tooltipBehaviorRenderer._isHovering = false;
    if (chart.zoomPanBehavior.enablePinching == true &&
      ...
      final events = _chartState._tapDownEvents;
      final currentEventPointer = _chartState.currentTapIndex;
      if (events.isNotEmpty) {
        final Offset position = renderBox.globalToLocal(event.position);
        if (events[0].pointer == currentEventPointer) {
          _chartState._trackballBehaviorRenderer
              .onTouchMove(position.dx, position.dy);
        }
        if (events.length > 1 && events[1].pointer == currentEventPointer) {
          _chartState._trackballBehaviorRenderer2
              .onTouchMove(position.dx, position.dy);
        }
      }
    }
  ...
  void _performPointerUp(PointerUpEvent event) {
    ...
    if (chart.trackballBehavior.enable) {
      ...
      if (_chartState._tapDownEvents.length == 1 &&
          _chartState._tapDownEvents[0].pointer == event.pointer) {
        const pointerUpEvent = PointerUpEvent(pointer: -1);
        _chartState._tapDownEvents[0] = pointerUpEvent;
      } else if (_chartState._tapDownEvents.length == 2) {
        const pointerUpEvent = PointerUpEvent(pointer: -1);
        if (_chartState._tapDownEvents[0].pointer == event.pointer) {
          _chartState._tapDownEvents[0] = pointerUpEvent;
        } else if (_chartState._tapDownEvents[1].pointer == event.pointer) {
          _chartState._tapDownEvents[1] = pointerUpEvent;
        }
      }
    }
    // ignore: unnecessary_null_comparison
  ...
  void _bindInteractionWidgets(
    ...
    _TrackballPainter trackballPainter2;
    ...
    if (chart.trackballBehavior != null && chart.trackballBehavior.enable) {
      if (chart.trackballBehavior.builder != null) {
        ...
        trackballPainter = _TrackballPainter(
          chartState: _chartState,
          behaviorRenderer: _chartState._trackballBehaviorRenderer,
          valueNotifier: _chartState._repaintNotifiers['trackball']!,
        );
        trackballPainter2 = _TrackballPainter(
            chartState: _chartState,
            behaviorRenderer: _chartState._trackballBehaviorRenderer2,
            valueNotifier: _chartState._repaintNotifiers['trackball2']!);
        _chartState._trackballBehaviorRenderer._trackballPainter =
            trackballPainter;
        _chartState._trackballBehaviorRenderer2._trackballPainter =
            trackballPainter2;
        ...
        userInteractionWidgets.add(Container(
            ...
            width: constraints.maxWidth,
            decoration: const BoxDecoration(color: Colors.transparent),
            child: CustomPaint(painter: trackballPainter2)));
      }
    }
    // ignore: unnecessary_null_comparison
