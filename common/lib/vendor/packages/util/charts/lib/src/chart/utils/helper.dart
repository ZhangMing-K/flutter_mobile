...
void _drawText(Canvas canvas, String? text, Offset point, TextStyle? style,
  ...
  final TextPainter tp = TextPainter(
      ...
      textDirection: TextDirection.ltr,
      textAlign: TextAlign.center,
      maxLines: maxLines as int?);
  tp.layout();
  canvas.save();
  canvas.translate(point.dx, point.dy);
  Offset labelOffset = const Offset(0.0, 0.0);
  if (angle != null && angle > 0) {
    canvas.rotate(_degreeToRadian(angle) as double);
...
Rect _rotatedTextSize(Size size, int angle) {
  ...
  final vector.Matrix2 _rotatorMatrix =
      vector.Matrix2.rotation(_degreeToRadian(angle) as double);
...
void _calculatePieIconPath(
  ...
  path.moveTo(x, y);
  path.lineTo(x + r, y);
  path.arcTo(Rect.fromCircle(center: Offset(x, y), radius: r as double),
      _degreesToRadians(0) as double, _degreesToRadians(270) as double, false);
  path.close();
  path.moveTo(x + width / 10, y - height / 10);
  path.lineTo(x + r, y - height / 10);
  path.arcTo(Rect.fromCircle(center: Offset(x + 2, y - 2), radius: r),
      _degreesToRadians(-5) as double, _degreesToRadians(-80) as double, false);
  path.close();
}
...
Rect _calculateRectangle(num? x1, num? y1, num? x2, num? y2,
  ...
  return Rect.fromLTWH(
      math.min(point1.x as double, point2.x as double),
      math.min(point1.y as double, point2.y as double),
      (point2.x! - point1.x!).abs() as double,
      (point2.y! - point1.y!).abs() as double);
}
  ...
  return Rect.fromLTWH(
      isColumn || isRangeColumn || isHistogram
          ...
          : isStackedColumn
              ? _chartState._chartAxis!._axisClipRect!.left
              : isStackedBar
                  ? math.min<num>(point1.x!, point2.x!) +
                      (-stackedBarSeries!.trackBorderWidth -
                          stackedBarSeries.trackPadding)
                  : math.min<num>(point1.x!, point2.x!) +
                      (-barSeries!.trackBorderWidth - barSeries.trackPadding),
      isColumn
          ? (math.min<num>(point1.y!, point2.y!) -
      ...
      if (seriesRenderer != null && seriesRenderer is _StackedSeriesRenderer) {
        ...
        if (groupName != null) {
          ...
          if (stackingGroupPos.isEmpty) {
            seriesRenderer._rectPosition = rectCount;
            stackingGroupPos.add(_StackingGroup(groupName, rectCount++ as int));
            ...
            for (int j = 0; j < stackingGroupPos.length; j++) {
              if (groupName == stackingGroupPos[j].groupName) {
                ...
                seriesRenderer._rectPosition = rectCount;
                stackingGroupPos
                    .add(_StackingGroup(groupName, rectCount++ as int));
                break;
              }
            }
          }
...
double? _pointToXValue(bool? _requireInvertedAxis,
    ChartAxisRenderer? axisRenderer, Rect? rect, double x, double y) {
  if (axisRenderer != null) {
    if (!_requireInvertedAxis!) {
      return _coefficientToValue(x / rect!.width, axisRenderer) as double?;
    }
    return _coefficientToValue(1 - (y / rect!.height), axisRenderer) as double?;
  }
  return double.nan;
}
...
// ignore: unused_element
double? _pointToYValue(bool? _requireInvertedAxis,
    ChartAxisRenderer? axisRenderer, Rect? rect, double x, double y) {
  if (axisRenderer != null) {
    if (!_requireInvertedAxis!) {
...
void _needsRepaintChart(
  ...
  if (_chartState._chartSeries!.visibleSeriesRenderers.length ==
    ...
    _chartState._seriesRenderers.forEach(
...
Path _getMarkerShapesPath(DataMarkerType? markerType, Offset position,
    Size size, CartesianSeriesRenderer seriesRenderer,
    [int? index,
  ...
  if (seriesRenderer._visible) {
    ...
    if (seriesRenderer._dataPoints.isNotEmpty) {
      ...
      point1 = _calculatePoint(
          ...
          _series,
          rect!);
      _path.moveTo(point1.x as double, point1.y as double);
      _strokePath.moveTo(point1.x as double, point1.y as double);
      for (int pointIndex = 0;
        ...
        if (point!.isVisible) {
          point1 = _calculatePoint(
              ...
              _series,
              rect);
          _points.add(Offset(point1.x as double, point1.y as double));
          _path.lineTo(point1.x as double, point1.y as double);
          _strokePath.lineTo(point1.x as double, point1.y as double);
        } else {
          if (_series.emptyPointSettings.mode != EmptyPointMode.drop) {
            for (int j = pointIndex - 1; j >= startPoint; j--) {
              point2 = _calculatePoint(
                  ...
                  _series,
                  rect);
              _path.lineTo(point2.x as double, point2.y as double);
              if (_series.borderDrawMode == BorderDrawMode.excludeBottom) {
                _strokePath.lineTo(point1.x as double, point2.y as double);
              } else if (_series.borderDrawMode == BorderDrawMode.all) {
                _strokePath.lineTo(point2.x as double, point2.y as double);
              }
            }
...
List<num?>? _getMonotonicSpline(List<num?> xValues, List<num?> yValues,
  ...
  final List<num?> slope = <num?>[]..length = count - 1; //  List<num?>();
  final List<num?> coefficient = <num?>[]..length = count; // List<num?>(count);
...
List<num?> _getCardinalSpline(List<num?> xValues, List<num?> yValues,
  ...
  final List<num?> tangentsX = <num?>[]..length = (count);
...
List<num?> _naturalSpline(List<num?> xValues, List<num?> yValues,
  ...
  const double a = 6;
  final int count = dataCount;
  final List<num?> yCoef = <num?>[]..length = (count);

  final List<num?> u = <num?>[]..length = (count);
...
List<double?> _calculateMonotonicControlPoints(
  ...
  final List<double?> values = <double?>[]..length = (4);
...
List<double?> _calculateControlPoints(List<num?> xValues, List<num?> yValues,
  ...
  final List<double?> controlPoints = <double?>[]..length = 4;
  ...
  controlPoints[0] = startControlPointsX as double?;
  controlPoints[1] = startControlPointsY as double?;
  controlPoints[2] = endControlPointsX as double?;
  controlPoints[3] = endControlPointsY as double?;
...
void _calculateSplineAreaControlPoints(CartesianSeriesRenderer seriesRenderer) {
  ...
  if (xValues.isNotEmpty) {
    final List<num?> dx = <num?>[]..length =
        xValues.length - 1; //  List<num?>(xValues.length - 1);
...
void _findSplineRangeAreaControlPoint(
  ...
  for (int pointIndex = 0; pointIndex < xValues.length - 1; pointIndex++) {
    ...
    if (xValues[pointIndex] != null &&
      ...
      final double x = xValues[pointIndex]!.toDouble();
      final double? low =
          seriesRenderer._dataPoints![pointIndex]!.low.toDouble();
      final double? high =
...
/// Get position given coordinates

Offset getPositionGivenRect(Rect rect, Offset pos) {
  //BRIAN SCHARDT ADDED
  double x = pos.dx;
  double y = pos.dy;
  if (pos.dx < rect.left) {
    x = rect.left;
  } else if (pos.dx >= rect.right) {
    x = rect.right;
  }

  if (pos.dy < rect.top) {
    y = rect.top;
  } else if (pos.dy >= rect.bottom) {
    y = rect.bottom;
  }
...
num? _getCrossesAtValue(
  ...
  for (int i = 0; i < axisCollection.length; i++) {
    if (chart._chartSeries!.visibleSeriesRenderers[seriesIndex as int]!
            ._xAxisRenderer!._name ==
