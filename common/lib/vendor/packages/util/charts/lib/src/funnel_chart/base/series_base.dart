...
class _FunnelSeries {
  ...
  List<FunnelSeriesRenderer?> visibleSeriesRenderers =
  ...
  void _findVisibleSeries() {
    ...
    final ChartIndexedValueMapper<dynamic>? xValue =
        currentSeries!.xValueMapper;
    final ChartIndexedValueMapper<dynamic>? yValue =
        currentSeries!.yValueMapper;
  ...
  void _setPointStyle(FunnelSeriesRenderer seriesRenderer) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      currentPoint.borderColor =
          currentPoint.isEmpty ? empty.borderColor : currentSeries!.borderColor;
      currentPoint.borderWidth =
          currentPoint.isEmpty ? empty.borderWidth : currentSeries!.borderWidth;
  ...
  void _initializeSizeRatio(FunnelSeriesRenderer seriesRenderer,
    ...
    for (num i = points.length - 1; i >= 0; i--) {
      ...
      if (points[index as int].isVisible) {
  ...
  void _calculatePathSegment(String? seriesType, PointInfo<dynamic> point) {
    ...
    final num x = (pathRegion[0].dx + pathRegion[bottom as int].dx) / 2;
    final num right = (pathRegion[1].dx + pathRegion[bottom - 1].dx) / 2;
    point.region = Rect.fromLTWH(x as double, pathRegion[0].dy, right - x,
  ...
  void _pointExplode(num pointIndex) {
    ...
    final PointInfo<dynamic> point =
        seriesRenderer._renderPoints![pointIndex as int];
  ...
  void _calculateFunnelPathRegion(
    ...
    currentPoint.pathRegion.add(Offset(line1X as double, line1Y as double));
    currentPoint.pathRegion.add(Offset(line2X as double, line2Y as double));
    currentPoint.pathRegion.add(Offset(line3X as double, line3Y as double));
    currentPoint.pathRegion.add(Offset(line4X as double, line4Y as double));
    currentPoint.pathRegion.add(Offset(line5X as double, line5Y as double));
    currentPoint.pathRegion.add(Offset(line6X as double, line6Y as double));
  ...
  void _seriesPointSelection(num? pointIndex, ActivationMode mode) {
    ...
    if (seriesRenderer._isSelectionEnable && mode == chart.selectionGesture) {
      ...
      if (!isPointAlreadySelected) {
        selectionData.add(pointIndex as int?);
        _chartState._seriesRepaintNotifier!.value++;
      }
    }
  }
  ...
  SelectionArgs? _getSelectionEventArgs(
    ...
    if (series != null) {
      ...
      _selectionArgs = SelectionArgs(seriesRenderer, seriesIndex as int?,
          pointIndex as int?, pointIndex as int?);
      _selectionArgs!.selectedBorderColor =
