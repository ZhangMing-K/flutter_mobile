...
class TrendlineRenderer {
  ...
  List<CartesianChartPoint<dynamic>> getExponentialPoints(
    ...
    ptsExpo.add(getDataPoint(
        x2, y2, points[midPoint - 1 as int], seriesRenderer, ptsExpo.length));
    ptsExpo.add(getDataPoint(
  ...
  List<CartesianChartPoint<dynamic>> getPowerPoints(
    ...
    ptsPow.add(getDataPoint(
        x2, y2, points[midPoint - 1 as int], seriesRenderer, ptsPow.length));
    ptsPow.add(getDataPoint(
  ...
  List<CartesianChartPoint<dynamic>> getLogarithmicPoints(
    ...
    ptsLog.add(getDataPoint(
        x2, y2, points[midPoint - 1 as int], seriesRenderer, ptsLog.length));
    ptsLog.add(getDataPoint(
  ...
  List<CartesianChartPoint<dynamic>> _getPolynomialPoints(
    ...
    _polynomialSlopes = []..length = (_trendline.polynomialOrder + 1);
    ...
    final List<dynamic> numArray = []..length =
        (2 * _trendline.polynomialOrder + 1);
    final List<dynamic> matrix = []..length = (_trendline.polynomialOrder + 1);

    for (int i = 0; i <= _trendline.polynomialOrder; i++) {
      matrix[i] = []..length = (_trendline.polynomialOrder + 1);
    }
    ...
    return pts as List<CartesianChartPoint<dynamic>>;
  }
    ...
    if (seriesRenderer._series!.trendlines != null && _pointsData != null) {
      for (int i = 0; i < _pointsData!.length; i++) {
        if (_pointsData![i]!.x != null && _pointsData![i]!.y != null) {
          final _ChartLocation currentChartPoint =
              _pointsData![i]!.markerPoint = _calculatePoint(
                  (seriesRenderer._xAxisRenderer is DateTimeAxisRenderer)
                  ...
                  rect);
          _points.add(Offset(
              currentChartPoint.x as double, currentChartPoint.y as double));
  ...
  bool _gaussJordanElimination(
    ...
    final List<dynamic> numArray1 = []..length = length;
    final List<dynamic> numArray2 = []..length = length;
    final List<dynamic> numArray3 = []..length = length;
  ...
  List<Offset> getPolynomialCurve(
    ...
    for (dynamic x = start;
      ...
      final _ChartLocation position = _calculatePoint(
          ...
          seriesRenderer._series,
          _chartState._chartAxis!._axisClipRect!);
      polyPoints.add(Offset(position.x as double, position.y as double));
    }
    return polyPoints;
  }
