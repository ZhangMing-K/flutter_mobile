...
class _TrackballPainter extends CustomPainter {
  _TrackballPainter({required this.chartState, this.valueNotifier})
  ...
  void _drawTrackball(Canvas canvas) {
    ...
    for (int index = 0; index < chartPointInfo.length; index++) {
      ...
      if (_markerShapes.isNotEmpty && _markerShapes.length > index) {
  ...
  _TooltipPositions _verticalArrangements(
      _TooltipPositions tooltipPPosition,
      List<ChartAxisRenderer?> _xAxesInfo,
      List<ChartAxisRenderer?> _yAxesInfo) {
    final _TooltipPositions tooltipPosition = tooltipPPosition;
    num chartHeight = 0, startPos = 0;
    ...
    for (i = 0; i < endPoint; i++) {
      ...
      if (temp >= tooltipTop[i + 1]) {
        ...
        if (count - 1 == endPoint - 1 || i == endPoint - 1) {
          ...
          midPos =
              (visibleLocation[startPoint as int]! + visibleLocation[i + 1]!) /
                  2;
          tempTooltipHeight =
        ...
        if (count > 0) {
          halfHeight = (temp - start) / 2;
          midPos =
              (visibleLocation[startPoint as int]! + visibleLocation[i]!) / 2;
          tempTooltipHeight =
              tooltipBottom[startPoint] - tooltipTop[startPoint];
          tooltipTop[startPoint] = midPos - halfHeight;
          tooltipBottom[startPoint] =
    ...
    if (chart.trackballBehavior.tooltipDisplayMode ==
      ...
      for (int i = 0; i < chartPointInfo.length; i++) {
        ...
        final String? seriesType =
            chartPointInfo[i].seriesRenderer!._seriesType;
        if (chartPointInfo[i].seriesRenderer!._isIndicator &&
            chartPointInfo[i]
                .seriesRenderer!
                ._series!
      ...
      if (chartPointInfo[index]
        ...
        x = chartPointInfo[index].chartDataPoint!.markerPoint!.x as double?;
        y = chartPointInfo[index].chartDataPoint!.markerPoint!.y as double?;
  ...
  void _calculateTrackballRect(
    ...
    labelRect = chart.trackballBehavior.tooltipDisplayMode ==
        ...
        : Rect.fromLTWH(xPos!, tooltipPosition!.tooltipTop[index] as double,
    ...
    if (chart.trackballBehavior.lineType == TrackballLineType.vertical) {
      if (isRectSeries || chart.isTransposed) {
        ...
        dashArrayPath.moveTo(
            chartState._trackballBehaviorRenderer!._position.dx,
            chartState._chartAxis!._axisClipRect!.top);
      ...
      if (chart.trackballBehavior.tooltipDisplayMode !=
        ...
        if (isHorizontalOrientation) {
          ...
          if (isRight) {
            ...
            backgroundPath.lineTo(
                rectF.right + pointerLength!, yPosition as double);
            ...
            backgroundPath.lineTo(
                rectF.left - pointerLength!, yPosition as double);
          ...
          for (final CartesianSeriesRenderer axisSeriesRenderer
            ...
            if (cartesianSeriesRenderer._dataPoints!.isNotEmpty) {
              ...
              for (final CartesianChartPoint<dynamic> dataPoint
                ...
                final num? cumulativeValue = cartesianSeriesRenderer
                    ._dataPoints![index]!.cumulativeValue;
                ...
                if (!xPos!.toDouble().isNaN) {
                  ...
                  if (cartesianSeriesRenderer._seriesType!.contains('range') ||
                    ...
                    if (cartesianSeriesRenderer._seriesType ==
                            'hiloopenclose' ||
                        cartesianSeriesRenderer._seriesType == 'candle') {
                      openXPos = dataPoint.openPoint!.x as double?;
                      openYPos = dataPoint.openPoint!.y as double?;
                      closeXPos = dataPoint.closePoint!.x as double?;
                      closeYPos = dataPoint.closePoint!.y as double?;
                    }
                    ...
                    lowerXPos = dataPoint.lowerQuartilePoint!.x as double?;
                    lowerYPos = dataPoint.lowerQuartilePoint!.y as double?;
                    upperXPos = dataPoint.upperQuartilePoint!.x as double?;
                    upperYPos = dataPoint.upperQuartilePoint!.y as double?;
                  }
  ...
  List<_ChartPointInfo> _getRectSeriesPointInfo(
    ...
    if (pointInfo.seriesRenderer!._isRectSeries &&
      ...
      final double xStartValue = xValue! + sideBySideInfo.minimum as double;
      final double xEndValue = xValue + sideBySideInfo.maximum as double;
  ...
  void _validateNearestXValue(
    ...
    double nearPointX = seriesRenderer._chartState!._requireInvertedAxis!
        ? axisClipRect.top
        : axisClipRect.left;
    final double touchXValue = seriesRenderer._chartState!._requireInvertedAxis!
        ? touchYPos
        : touchXPos;
    double delta = 0;
  ...
  void showTrackball(
    ...
    if (pointIndex.abs() < seriesRenderer._dataPoints!.length) {
      ...
      position = _calculatePoint(
          ...
          seriesRenderer._series,
          rect!);
      _generateAllPoints(Offset(position.x as double, position.y as double));
    }
  }
