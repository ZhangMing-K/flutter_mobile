    ...
    if (_series!.animationDuration > 0 &&
      ...
      if (!_seriesRenderer!._chartState!._widgetNeedUpdate ||
        ...
        if (_isTransposed!) {
          ...
          canvas.drawLine(Offset(_lowX as double, _lowPoint!.y as double),
              Offset(_highX as double, _highPoint!.y as double), strokePaint!);
        } else {
          _centerY = _high! + ((_low! - _high!) / 2);
          _highY = _centerY! - ((_centerY! - _high!) * animationFactor!);
          _lowY = _centerY! + ((_low! - _centerY!) * animationFactor!);
          canvas.drawLine(Offset(_lowPoint!.x as double, _highY as double),
              Offset(_highPoint!.x as double, _lowY as double), strokePaint!);
        }
      } else {
        _currentSegment =
      ...
      if (_series!.dashArray[0] != 0 && _series!.dashArray[1] != 0) {
        _path = Path();
        _path!.moveTo(_lowPoint!.x as double, _high as double);
        _path!.lineTo(_highPoint!.x as double, _low as double);
        _drawDashedLine(canvas, _series!.dashArray, strokePaint, _path);
      } else {
        canvas.drawLine(Offset(_lowPoint!.x as double, _high as double),
            Offset(_highPoint!.x as double, _low as double), strokePaint!);
      }
    }
  }
}
