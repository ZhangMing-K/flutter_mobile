...
class _ChartLegend {
  ...
  Legend? legend;
  late List<_LegendRenderContext> legendCollections;
  int? rowCount;
  int? columnCount;
    ...
    if (legend!.isVisible) {
      ...
      if (legendCollections.isNotEmpty ||
        ...
        if (legend!.title.text != null && legend!.title.text!.isNotEmpty) {
          titleSize = _measureText(legend!.title.text, legend!.title.textStyle);
        ...
        for (int i = 0; i < length; i++) {
          if (isTemplate) {
            ...
            textSize = _measureText(legendText, legend!.textStyle);
            ...
            legendRenderContext.size =
                Size(currentWidth as double, currentHeight as double);
          }
          if (i == 0) {
        ...
        legendSize = Size(maxLegendWidth! + (padding as double),
  ...
  void _calculateLegends(SfCartesianChart? chart, int index,
    ...
    late TrendlineRenderer trendlineRenderer;
...
class _LegendContainer extends StatelessWidget {
  _LegendContainer({required this.chartState}) : chart = chartState._chart;
  ...
  Widget build(BuildContext context) {
    ...
    if (needLegendTitle) {
      titleHeight =
          _measureText(legend.title.text, legend.title.textStyle).height + 10;
    }
    ...
    if (chartState._chartLegend.isNeedScrollable) {
      widget = SizedBox(
          ...
          child: SingleChildScrollView(
              scrollDirection: legedRenderer!._orientation ==
                      LegendItemOrientation.horizontal
                  ? Axis.vertical
                  : Axis.horizontal,
      ...
      widget = SizedBox(
          ...
          child: Wrap(
              direction: legedRenderer!._orientation ==
                      LegendItemOrientation.horizontal
                  ? Axis.horizontal
                  : Axis.vertical,
