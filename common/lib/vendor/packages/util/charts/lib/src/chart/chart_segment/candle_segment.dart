...
class CandleSegment extends ChartSegment {
  ...
  void calculateSegmentPoints() {
    ...
    _highPoint!.x = (_showSameValue! && _isTransposed!)
        ? _highPoint!.x! + 2
        : _highPoint!.x;
    _low = _lowPoint!.y =
        (_showSameValue! && !_isTransposed!) ? _lowPoint!.y! - 2 : _lowPoint!.y;
    _high = _highPoint!.y = (_showSameValue! && !_isTransposed!)
        ? _highPoint!.y! + 2
        : _highPoint!.y;
  ...
  void _drawRectPath() {
    _path!.moveTo(!_isTransposed! ? _openX as double : _topRectY as double,
        !_isTransposed! ? _topRectY as double : _closeY as double);
    _path!.lineTo(!_isTransposed! ? _closeX as double : _topRectY as double,
        !_isTransposed! ? _topRectY as double : _openY as double);
    _path!.lineTo(!_isTransposed! ? _closeX as double : _bottomRectY as double,
        !_isTransposed! ? _bottomRectY as double : _openY as double);
    _path!.lineTo(!_isTransposed! ? _openX as double : _bottomRectY as double,
        !_isTransposed! ? _bottomRectY as double : _closeY as double);
    _path!.lineTo(!_isTransposed! ? _openX as double : _topRectY as double,
        !_isTransposed! ? _topRectY as double : _closeY as double);
    _path!.close();
  }

  void _drawLine(Canvas canvas) {
    canvas.drawLine(Offset(_centerHigh as double, _topRectY as double),
        Offset(_centerHigh as double, _topLineY as double), fillPaint!);
    canvas.drawLine(Offset(_centerHigh as double, _bottomRectY as double),
        Offset(_centerHigh as double, _bottomLineY as double), fillPaint!);
  }

  void _drawFillLine(Canvas canvas) {
    final bool isOpen = _currentPoint!.open > _currentPoint!.close;
    canvas.drawLine(
        Offset(_topRectY as double, _highY as double),
        Offset(
            _topRectY! +
        ...
        fillPaint!);
    canvas.drawLine(
        Offset(_bottomRectY as double, _highY as double),
        Offset(
            _bottomRectY! -
  ...
  void onPaint(Canvas canvas) {
    if (fillPaint != null &&
      ...
      _centersY = _closeY! + ((_closeY! - _openY!).abs() / 2);
      _topRectY =
          _centersY! - ((_centersY! - _closeY!).abs() * animationFactor!);
      _topLineY =
          _topRectY! - ((_topRectY! - _highY!).abs() * animationFactor!);
      _bottomRectY =
          _centersY! + ((_centersY! - _openY!).abs() * animationFactor!);
      _bottomLineY =
      ...
      if (_isTransposed!) {
        ...
        if (_showSameValue!) {
          canvas.drawLine(
              Offset(
                  _centerHighPoint!.x as double, _centerHighPoint!.y as double),
              Offset(
                  _centerLowPoint!.x as double, _centerHighPoint!.y as double),
              fillPaint!);
        } else {
          _path!.moveTo(_topRectY as double, _highY as double);
          _centerHigh! < _closeX!
              ? _path!.lineTo(
                  _topRectY! -
                      ((_closeX! - _centerHigh!).abs() * animationFactor!),
                  _highY as double)
              : _path!.lineTo(
                  _topRectY! +
                      ((_closeX! - _centerHigh!).abs() * animationFactor!),
                  _highY as double);
          _path!.moveTo(_bottomRectY as double, _highY as double);
          _centerLow! > _openX!
              ? _path!.lineTo(
                  _bottomRectY! +
        ...
        _openX == _closeX
            ? canvas.drawLine(Offset(_openX as double, _openY as double),
                Offset(_closeX as double, _closeY as double), fillPaint!)
            : _drawRectPath();
      } else {
        if (_currentPoint!.open > _currentPoint!.close) {
          final num? temp = _closeY;
        ...
        _showSameValue!
            ? canvas.drawLine(
                Offset(_centerHighPoint!.x as double, _highPoint!.y as double),
                Offset(_centerHighPoint!.x as double, _lowPoint!.y as double),
                fillPaint!)
            : _drawLine(canvas);
        ...
        if (fillPaint!.style == PaintingStyle.fill) {
          if (_isTransposed!) {
            if (_currentPoint!.open > _currentPoint!.close) {
              _showSameValue!
                  ? canvas.drawLine(
                      Offset(_centerHighPoint!.x as double,
                          _centerHighPoint!.y as double),
                      Offset(_centerLowPoint!.x as double,
                          _centerHighPoint!.y as double),
                      fillPaint!)
            ...
            _showSameValue!
                ? canvas.drawLine(
                    Offset(
                        _centerHighPoint!.x as double, _highPoint!.y as double),
                    Offset(
                        _centerHighPoint!.x as double, _lowPoint!.y as double),
                    fillPaint!)
                : _drawLine(canvas);
          }
        }
