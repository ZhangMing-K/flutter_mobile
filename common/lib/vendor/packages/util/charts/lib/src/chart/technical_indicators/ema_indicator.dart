...
class EmaIndicator<T, D> extends TechnicalIndicators<T, D> {
  ...
  void _initDataSource(TechnicalIndicators<dynamic, dynamic> indicator,
    ...
    if (validData.isNotEmpty &&
        ...
        indicator.period > 0) {
      _calculateEMAPoints(indicator as EmaIndicator<dynamic, dynamic>,
  ...
  void _calculateEMAPoints(
    ...
    if (validData.length >= period && period > 0) {
      ...
      point = technicalIndicatorsRenderer._getDataPoint(
          validData[period - 1 as int]!.x,
          average,
          validData[period - 1 as int]!,
      ...
      while (index < validData.length) {
        if (validData[index as int]!.isVisible ||
            validData[index]!.isGap == true ||
            validData[index]!.isDrop == true) {
          final num prevAverage = points[index - (period as int)]!.y;
