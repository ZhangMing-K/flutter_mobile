...
// ignore:unused_element
  ...
  List<dynamic> _stochasticCalculation(
    ...
    if (data.length >= period + kPeriod && kPeriod > 0) {
      ...
      while (length >= count) {
        num sum = 0;
        for (int i = period - 1 as int; i < (period + kPeriod - 1); i++) {
          sum = sum + temp[i]!;
        }
        sum = sum / kPeriod;
  ...
  List<CartesianChartPoint<dynamic>> _calculatePeriod(
      ...
      TechnicalIndicatorsRenderer technicalIndicatorsRenderer) {
    final List<num?> lowValue = <num?>[]..length = (data.length);
    final List<num?> highValue = <num?>[]..length = (data.length);
    final List<num?> closeValue = <num?>[]..length = (data.length);
    ...
    if (data.length > period) {
      ...
      num? min, max;
      for (int i = period - 1 as int; i < data.length; ++i) {
