...
class BubbleSegment extends ChartSegment {
  ...
  void calculateSegmentPoints() {
    ...
    _currentPoint!.region = Rect.fromLTRB(
        localtion.x! - 2 * (_radius as double),
        localtion.y! - 2 * (_radius as double),
        localtion.x! + 2 * (_radius as double),
        localtion.y! + 2 * (_radius as double));
  ...
  void onPaint(Canvas canvas) {
    ...
    if (_seriesRenderer!._chartState!._widgetNeedUpdate &&
      ...
      canvas.drawCircle(Offset(_centerX as double, _centerY as double),
          _radius! * animationFactor!, fillPaint!);
      canvas.drawCircle(Offset(_centerX as double, _centerY as double),
          _radius! * animationFactor!, strokePaint!);
    }
  }
}
