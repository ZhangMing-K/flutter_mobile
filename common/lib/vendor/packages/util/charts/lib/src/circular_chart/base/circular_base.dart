...
class SfCircularChartState extends State<SfCircularChart>
  ...
  void _redraw() {
    ...
    if (_isLegendToggled!) {
      ...
      _oldPoints = <ChartPoint<dynamic>?>[]..length =
          (_prevSeriesRenderer!._renderPoints!.length);
      ...
      for (int i = 0; i < chart.annotations!.length; i++) {
        ...
        if (annotation.widget != null) {
          ...
          final double annotationHeight = _percentToValue(
              annotation.height, chartState!._chartSeries!.size / 2) as double;
          final double? annotationWidth = _percentToValue(
              annotation.width, chartState!._chartSeries!.size / 2) as double?;
