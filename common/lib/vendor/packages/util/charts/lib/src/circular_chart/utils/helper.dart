...
Path _getArcPath(num innerRadius, num radius, Offset center, num startAngle,
  ...
  if (isFullCircle) {
    path.arcTo(Rect.fromCircle(center: center, radius: radius as double),
        startAngle as double, midpointAngle - startAngle, true);
    path.arcTo(Rect.fromCircle(center: center, radius: radius),
        midpointAngle as double, endAngle - midpointAngle, true);
  } else {
    path.arcTo(Rect.fromCircle(center: center, radius: radius as double),
        startAngle as double, degree as double, true);
  }

  path.lineTo(innerRadiusEndPoint.x, innerRadiusEndPoint.y);

  if (isFullCircle) {
    path.arcTo(Rect.fromCircle(center: center, radius: innerRadius as double),
        endAngle as double, midpointAngle - endAngle, true);
    path.arcTo(Rect.fromCircle(center: center, radius: innerRadius),
        midpointAngle as double, startAngle - midpointAngle, true);
  } else {
    path.arcTo(Rect.fromCircle(center: center, radius: innerRadius as double),
        endAngle as double, startAngle - endAngle, true);
  }
...
Path _getRoundedCornerArcPath(num innerRadius, num outerRadius, Offset center,
  ...
  if (cornerStyle == CornerStyle.startCurve ||
    ...
    path.addArc(
        ...
        midStartAngle as double,
        midEndAngle as double);
  }

  path.addArc(
      Rect.fromCircle(center: center, radius: outerRadius as double),
      _degreesToRadians(startAngle) as double,
      _degreesToRadians(endAngle - startAngle) as double);
  ...
  path.arcTo(
      Rect.fromCircle(center: center, radius: innerRadius as double),
      _degreesToRadians(endAngle) as double,
      _degreesToRadians(startAngle) - (_degreesToRadians(endAngle) as double),
      false);
...
void _drawPath(Canvas canvas, _StyleOptions style, Path path) {
  ...
  if (style.strokeColor != null &&
    ...
    paint.strokeWidth = style.strokeWidth as double;
