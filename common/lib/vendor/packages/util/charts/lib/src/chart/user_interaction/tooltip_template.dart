...
class _TooltipTemplateState extends State<_TooltipTemplate>
  ...
  late BuildContext tooltipContext;

  late bool needMeasure;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.show!) {
      if (needMeasure) {
        ...
        final Offset arrowLocation = Offset(tooltipLocation.dx,
            isOutOfBoundInTop ? top as double : top - arrowHeight);
        top = isOutOfBoundInTop ? top + arrowHeight : tooltipRect.top;
        tooltipWidget = Stack(children: <Widget>[
          CustomPaint(
              size: const Size(arrowHeight, arrowWidth),
  ...
  void _loaded() {
    final RenderBox renderBox = tooltipContext.findRenderObject() as RenderBox;
...
class _ArrowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Path path = Path()
      ..moveTo(centerX + (_isOutOfBoundInTop ? 0 : -arrowWidth as double),
          locationY as double)
      ..lineTo(
          centerX +
              (_isOutOfBoundInTop
                  ? -arrowWidth as double
                  : arrowWidth as double),
          locationY + (_isOutOfBoundInTop ? arrowHeight : 0))
      ..lineTo(centerX + (_isOutOfBoundInTop ? arrowWidth as double : 0),
