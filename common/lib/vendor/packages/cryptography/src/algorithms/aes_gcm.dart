...
class AesGcm extends AesCipher {
  ...
  Uint8List _mac(
    ...
    final macArgument = _macDataFrom(aad, cipherText as Uint8List);
    final macAsUint8List = _ghash(h as Uint8List, macArgument);
  ...
  static Uint8List _macDataFrom(List<int>? aad, Uint8List cipherText) {
    aad ??= const <int>[];
    final aadPaddedLength = (aad.length + 15) ~/ 16 * 16;
    final cipherTextPaddedLength = (cipherText.length + 15) ~/ 16 * 16;
