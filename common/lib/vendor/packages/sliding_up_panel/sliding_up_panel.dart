...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  createContoller() {
    _ac = AnimationController(
      ...
      ..addListener(() {
        changed = false;
        if (_ac.value > 0 && closed == true) {
          closed = false;
        }
        if (_ac.value == 0 && closed == false) {
          closed = true;
          changed = true;
        }
        ...
        if (widget.onPanelClosed != null && closed && changed) {
  ...
  @override
  void dispose() {
    _ac.dispose(); //THIS WAS COMMENTED OUT BY BRIAN SCHARDT
    _sc!.dispose();
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    return Listener(
      onPointerDown: (PointerDownEvent p) {
        ...
        return _vt.addPosition(p.timeStamp, p.position);
      },
      onPointerMove: (PointerMoveEvent p) {
        ...
        if (readyToMoveDown || p.delta.dy < 0) {
      ...
      onPointerUp: (PointerUpEvent p) {
        return _onGestureEnd(_vt.getVelocity());
      },
      child: child,
    );
  }
