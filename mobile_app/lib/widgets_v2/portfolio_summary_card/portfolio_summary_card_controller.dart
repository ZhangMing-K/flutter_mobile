...
class PortfolioSummaryController extends GetxController {
  ...
  PortfolioSummaryController({required this.portfolio});
  ...
  void reconnect() {
    final value = portfolio?.value;
    if (value == null) return;
    Get.toNamed(Paths.ConnectInstitution.createPath([
      describeEnum(value.brokerName!),
      value.portfolioKey,
    ]));
  }
    ...
    if (list.contains(portfolio!.value?.authUserFollowInfo!.followStatus)) {
      ...
      if (portfolio!.value?.accountId != null ||
          portfolio!.value?.authUserFollowInfo!.followStatus ==
              FOLLOW_STATUS.APPROVED) {
        Get.toNamed(
            Paths.Portfolio.createPath([portfolio!.value?.portfolioKey]),
            id: 1);
      } else if (portfolio!.value?.accountId == null &&
          portfolio!.value?.authUserFollowInfo!.followStatus ==
              FOLLOW_STATUS.ME &&
          portfolio!.value?.brokerName == BROKER_NAME.TD_AMERITRADE) {
        Get.toNamed(
