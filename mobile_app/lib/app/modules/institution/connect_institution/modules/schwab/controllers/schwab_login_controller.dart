...
class SchwabLoginController extends GetxController {
  ...
  SchwabLoginController({this.portfolioKey, int? selectedTab}) {
  ...
  void cancelAddAccounts() {
    Get.back();
  }

  Future<void> complete({int? portfolioKey}) async {
  ...
  Future<void> connect({bool? navigateWhenLinked}) async {
    try {
  ...
  String formatDate(String datePartial) {
  ...
  Future<void> getPortfolioLogin() async {
    if (portfolioKey == null) {
      return;
    }
    final String brokerUsernameKey = 'iris-username-portfolio-$portfolioKey';
  ...
  void handleError(String message) {
    if (message.contains('frequent')) {
      Get.dialog(IrisDialog(
          title:
              'Schwab is having issues sending verification codes for your account',
          onConfirm: () {
            Get.back();
          },
          ...
          asyncFunction: () async {
            try {
              return await institutionService.connect(
                  institutionName: INSTITUTION_NAME.SCHWAB,
