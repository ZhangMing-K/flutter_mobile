...
class EtradeAddAccounts extends GetView<EtradeLoginController> {
  const EtradeAddAccounts({Key? key}) : super(key: key);
    ...
    return Obx(() {
      ...
      if (institutionAccounts!.isEmpty) {
        return Column(
          children: const [
            ...
            Text(
              'No brokerage accounts on Etrade to connect.',
              style: TextStyle(fontSize: 23),
            ),
            SizedBox(height: 30),
            Text(
              'Or you must disable 2 factor auth to connect',
              style: TextStyle(fontSize: 23),
            ),
          ],
        );
      ...
      return ListView.builder(
          ...
          itemBuilder: (context, i) {
            ...
            return checkAccountCard(institutionAccounts![i - 1]);
          });
    });
  }
  ...
  Widget checkAccountCard(InstitutionAccount account) {
    return Obx(() {
      var selected = controller.accountSelectedMap$.value[account.accountId]!;
